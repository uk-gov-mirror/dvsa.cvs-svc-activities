  ...
  context("calls activity service", () => {
    ...
    context("gets a successful response", () => {
      it("returns 201 and stringinfied empty string as body", async () => {
        GetActivityService.prototype.getActivities = jest.fn().mockResolvedValue({testResponse: 1234});
        const resp: HTTPResponse = await getActivity({pathParameters: {id: 1}}, ctx, () => { return; });
    ...
    context("gets an unsuccessful response", () => {
      it("returns the thrown  error", async () => {
        GetActivityService.prototype.getActivities = jest.fn().mockRejectedValue(new Error("Oh No!"));
        try {
          await getActivity({pathParameters: {id: 1}}, ctx, () => { return; });
        } catch (e) {
